...
enum class DriverType { Auto, DRV8833, TB6612 };

enum class ControlMode { Direct, Differential };

struct MotorPins {
	// DRV8833 style (both PWM on each side)
	uint8_t leftIn1 = 0, leftIn2 = 0, rightIn1 = 0, rightIn2 = 0;
	// TB6612 style (direction + PWM + optional standby)
	uint8_t leftPwm = 0, rightPwm = 0, stby = 0;

	static MotorPins drv8833(uint8_t l1, uint8_t l2, uint8_t r1, uint8_t r2) {
		MotorPins p; p.leftIn1 = l1; p.leftIn2 = l2; p.rightIn1 = r1; p.rightIn2 = r2; return p;
	}
	static MotorPins tb6612(uint8_t lIn1, uint8_t lIn2, uint8_t lPwm,
						  uint8_t rIn1, uint8_t rIn2, uint8_t rPwm, uint8_t standby) {
		MotorPins p; p.leftIn1 = lIn1; p.leftIn2 = lIn2; p.leftPwm = lPwm; p.rightIn1 = rIn1; p.rightIn2 = rIn2; p.rightPwm = rPwm; p.stby = standby; return p;
	}
};
...
public:
	MotorDriver();
	explicit MotorDriver(DriverType driver);
...
private:
	...
	void validatePinsCompileTime() const;
